<template>
  <ion-page>
    ...
    <!-- Modal "Más" -->
    <ion-modal :is-open="menuOpen" @didDismiss="menuOpen = false" :breakpoints="[0, 0.65]" :initial-breakpoint="0.65">
      <ion-header>
        <ion-toolbar>
          <ion-title>Módulos</ion-title>
          <ion-buttons slot="end">
            <ion-button @click="menuOpen = false"><ion-icon :icon="closeOutline" /></ion-button>
          </ion-buttons>
        </ion-toolbar>
      </ion-header>
      <ion-content class="ion-padding">
        <ion-list>
          ...
          <ion-item button @click="navigate('/tabs/users')" v-if="auth.isAdmin">
            <ion-icon :icon="personOutline"  slot="start" color="dark" />
            <ion-label>Usuarios</ion-label>
          </ion-item>
          <ion-item button @click="navigate('/tabs/perfil')">
